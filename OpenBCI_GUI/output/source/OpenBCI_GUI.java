...
public void toggleChannelImpedanceState(Button but, int Ichan, int code_P_N_Both) {
...
public void setChannelImpedanceState(int Ichan,boolean newstate,int code_P_N_Both) {
  if ((Ichan >= 0) && (Ichan < gui.impedanceButtonsP.length)) {
...
public void setBiasState(boolean state) {
  ...
  public void TraceDraw(Blank2DTrace.PlotRenderer pr) {
    ...
    if (dataX.length > 0) {       
      ...
      for (int Ichan = 0; Ichan < dataY.length; Ichan++) {
        ...
        pr.canvas.stroke((PApplet.parseInt((255/nchan)*Ichan)), 125-(PApplet.parseInt(((255/nchan)*Ichan)/2)), 255-(PApplet.parseInt((255/nchan)*Ichan)));
  ...
  public void writeRawData_dataPacket(DataPacket_ADS1299 data, float scale_to_uV, int nValsUsingScaleFactor) {
    ...
    if (output != null) {
      ...
      for (int Ival = 0; Ival < nVal; Ival++) {
        output.print(", ");
        if ((Ival >= nValsUsingScaleFactor) || (abs(scale_to_uV-1.0f) < 1e-6f)) {
          //do not scale the data
          output.print(Integer.toString(data.values[Ival]));
        } 
        else {
          //apply the scale factor
          output.print(String.format("%.2f", scale_to_uV * PApplet.parseFloat(data.values[Ival])));
        }
      }
