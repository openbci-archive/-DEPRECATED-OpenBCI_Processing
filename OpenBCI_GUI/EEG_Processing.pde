...
class EEG_Processing_User {
  ...
  int averagePeriod_L = 250;  //number of data packets to average over (250 = 1 sec)
  ...
  float acceptableLimitUV_L = 300;  //uV values above this limit are excluded, as a result of them almost certainly being noise...
  int uncounted_L = 0;
  //prez related
  ...
  int averagePeriod_R = 250;  //number of data packets to average over (250 = 1 sec)
  ...
  float acceptableLimitUV_R = 300;  //uV values above this limit are excluded, as a result of them almost certainly being noise... 
  int uncounted_R = 0;
  //prez related
  ...
  public void drawTriggerFeedback(){
    ...
    if(isRunning){
      ...
      if (eegProcessing_user.switchCounter_L == 1) {
        //draw red circle
        fill(255, 0, 0);
        ellipse(width/2-40, height/2, 20, 20);
        noFill();
        ellipse(width/2+40, height/2, 20, 20);
