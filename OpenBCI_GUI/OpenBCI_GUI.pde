...
String openBCI_portName[] = {"N/A", ""};      //do as array to support multiple boards.  Starts as N/A but is selected from control panel to match your OpenBCI USB Dongle's serial/COM
...
int nAuxPerOpenBCI = 3;   //number of auxillary data channels per OpenBCI unit
int nAuxTotal = 3;        //total number of auxillary data channels across all boards 
  ...
  if (useMultipleOpenBCIBoards) {
    ...
    if ((eegDataSource == DATASOURCE_NORMAL) || (eegDataSource == DATASOURCE_NORMAL_W_AUX)) {
      ...
      for (int Iboard = 0; Iboard < nEEGChannelsPerOpenBCI.length; Iboard++) {
        ...
        nAuxTotal += nAuxPerOpenBCI;
      }
      println("OpenBCI_GUI: initSystem: Multiple boards, nchan = " + str(nchan) + ", nAuxTotal = " + str(nAuxTotal));
    }
    ...
    for (int i=0; i<nDataBackBuff; i++) { 
      dataPacketBuff[i] = new DataPacket_ADS1299(nchan, nAuxTotal);
    }
    ...
    case DATASOURCE_NORMAL_W_AUX:
      ...
      openBCI = new OpenBCI_Multi(this, openBCI_portName[0], openBCI_baud, nEEGChannelsPerOpenBCI[0], useAux, nAuxPerOpenBCI); //this also starts the data transfer after XX seconds
      if (useMultipleOpenBCIBoards) {
        println("OpenBCI_GUI: adding 2nd OpenBCI board at " + openBCI_portName[1]);
        openBCI.addUnit(this, openBCI_portName[1], openBCI_baud, nEEGChannelsPerOpenBCI[1], useAux, nAuxPerOpenBCI);
      }
      break;
